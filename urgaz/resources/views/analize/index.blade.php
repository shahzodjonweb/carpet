    ...
    <table class="table mt-3" id="xsltable">
        ...
        <tbody>
          ...
          @endphp
                    ...
                    <td>{{ $check->volume }} ta </td>
                    <td>{{ $check->from }}</td>
                    <td class='text-danger'>-{{ $check->payment_amount }}</td>
                    <td>{{ $check->to }}</td>
                    <td class='text-success'>+{{ $check->payment_amount }}</td>
                    @switch($check->payment_type)
                        @case('cash')
                        <td class="text-primary">Naqt(so'm)</td>
                            @break
                            @case('perechisleniya')
                        <td class="text-primary">Naqt(so'm)</td>
                            @break
                        @case('terminal')
                        <td class="text-primary">Terminal</td>
                            @break
                            @case('valyuta')
                        <td class="text-primary">Valyuta(USD)</td>
                            @break
                        @default
